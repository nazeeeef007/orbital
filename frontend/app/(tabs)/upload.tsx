...
export default function UploadScreen() {
  ...
  const pickImage = useCallback(
    async (
      ...
      if (status !== "granted") {
        Alert.alert(
          "Permission Required",
          "Permission to access media library is needed to upload images!"
        );
        return;
      }
  ...
  const takePhoto = useCallback(
    async (
      ...
      if (status !== "granted") {
        Alert.alert(
          "Permission Required",
          "Permission to access camera is needed to take photos!"
        );
        return;
      }
  ...
  const handleAIAnalysis = async () => {
    if (!recipeText) {
      Alert.alert(
        "Missing Meal Description, please input the description of the meal before analysing"
      );
      return;
    }
  ...
  const uploadMeal = async () => {
    ...
    if (!calories || !protein || !carbs || !fat) {
      Alert.alert(
        "Validation Error",
        "Please fill in all **Nutritional Information** (Calories, Protein, Carbs, Fat)."
      );
      return;
    }
    if (!cuisine) {
    ...
    if (isHomecooked) {
      if (!prepTimeMins || parseInt(prepTimeMins) <= 0) {
        Alert.alert(
          "Validation Error",
          "Please provide a valid **Prep Time** in minutes for homecooked meals."
        );
        return;
      }
    } else {
      if (!price || parseFloat(price) <= 0) {
        Alert.alert(
          "Validation Error",
          "Please provide a valid **Price** for non-homecooked meals."
        );
        return;
      }
      if (!location.trim()) {
        Alert.alert(
          "Validation Error",
          "Please provide a **Location** for non-homecooked meals."
        );
        return;
      }
    }
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        ...
        >
          ...
          <Text style={styles.subtitle}>
            Share your meal and track its nutrition!
          </Text>
          ...
          <View style={styles.sectionCard}>
            <View style={styles.switchContainer}>
              ...
              <Switch
                ...
                thumbColor={
                  useAIForMacros ? Colors.secondary : Colors.cardBackground
                }
            ...
            {useAIForMacros && (
              <TouchableOpacity
                ...
                disabled={loading || !recipeText} // Disable if no meal description
              >
                {loading ? (
                  <ActivityIndicator
            ...
            {useAIForMacros && !recipeText && (
              <Text style={styles.aiHintText}>
                Add a meal description to enable AI analysis.
              </Text>
            )}
          </View>
          ...
          <View style={styles.sectionCard}>
            ...
            <View style={styles.switchContainer}>
              ...
              <Switch
                ...
                thumbColor={
                  isHomecooked ? Colors.success : Colors.cardBackground
                }
                ios_backgroundColor={Colors.border}
              />
            </View>
            ...
            {isHomecooked ? (
              <TextInput
                ...
                value={prepTimeMins}
                onChangeText={(text) =>
                  setPrepTimeMins(text.replace(/[^0-9]/g, ""))
                }
                style={styles.input}
                placeholderTextColor={Colors.placeholder}
              />
